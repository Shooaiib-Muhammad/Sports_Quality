...
foreach ($groupedData as $date => $data1) {
  $categoryData = array();
  $drilldownData = array();

  foreach ($data1 as $value) {
    $label = $value['label'];
    $defects = intval($value['defects']);

    $categoryData[] = array($label, $defects);
    $drilldownData[] = array($label, $defects);
  }

  $pivotDateName005[] = array('name' => $date, 'y' => array_sum(array_column($categoryData, 1)), 'drilldown' => $date);
  $drillDownDataPivot005[] = array('name' => $date, 'id' => $date, 'data' => $drilldownData);
}
...
<script>
  ...
  Highcharts.chart('pivot005', {
    ...
    drilldown: {
      series: <?php echo json_encode($drillDownDataPivot005, JSON_NUMERIC_CHECK); ?>
    },
    credits: {
      enabled: false
    },
