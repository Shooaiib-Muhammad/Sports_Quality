...
class Main extends CI_Controller
{
    ...
    {
        $data['data'] = $this->t->findallData();
        $data['dept'] = $this->d->getallDept();
        $data['team'] = $this->t->findTeam();
        $data['idelMints'] = $this->m->getidelmints();
        $data['idealmachines'] = $this->s->IdealMachines();
        
        $this->load->view('DMMS/Idealmachines', $data);
    }
    public function Ideamints()
    {
        $data['data'] = $this->t->findallData();
        $data['dept'] = $this->d->getallDept();
        $data['team'] = $this->t->findTeam();
        ...
        $this->load->view('DMMS/Idealmints', $data);
    }
    public function updatemachine($TID ,$OEEStatus,$BECStatus){
        $data = $this->m->updatemachine($TID, $OEEStatus, $BECStatus);
        return $this->output
    ...
    public function Dmms_Dashboard()
    {
        ...
        $data['rnd_count'] = $this->m->countrndInstalledMachines(29);
    ...
    {
        $data = $this->m->undoidealmints($TID);
        return $this->output
            ->set_content_type('application/json')
            ->set_status_header(200)
            ->set_output(json_encode($data));
    ...
    { {
            $data = $this->m->Assignidealmnts($Dept, $section, $idealmints, $Oeetarget);
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(200)
                ->set_output(json_encode($data));
    ...
    public function BEC()
    {
        ...
        $data['lfb_team'] = $this->t->countlfb(24);
        $data['packing_count'] = $this->m->countpackingInstalledMachines(25);
        $this->load->view('DMMS/BEC_Main', $data);
    }
}