...
class Teams extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('DMMS/Team_model', 'model');
		$this->load->model('DMMS/Department_model', 'd');
		$this->load->model('DMMS/Skill_model', 's');
...
$data['data'] = $this->model->findall($id);
				...
				$data['skills'] = $this->s->findall();
		$this->load->view('DMMS/new_team', $data);
		}
  ...
  $data['dept'] = $this->d->getallDept();
		...
		$this->load->view('DMMS/AssignTeam', $data);
	}
...
$data['title'] = "Airless Mini Team";
	...
	$data['getTeam'] = $this->model->getteamDetsils($id);
		$this->load->view('DMMS/teamdetails', $data);
}
}
