...
HAVING        (dbo.tbl_DMMS_Maintance.Date = CONVERT(DATE, '2021-11-09', 102)) AND (dbo.tbl_DMMS_Schedule.WorkStatus = 1)
");
        return $query->result_array();
    }
...
HAVING        (WorkStatus = 0) AND (SDate = CONVERT(DATE, '2021-11-09', 102))
");
        return $query->result_array();
    }
...
GROUP BY DeptName");
     ...
     public function MachineDowntime()
    {
        ...
        $query = $this->db->query("SELECT        SUM(Mints) AS Mints, DeptName, SecName + '  /  ' + Name AS Name
FROM            dbo.view_Dmms_History
WHERE        (Datee = '$Date')
