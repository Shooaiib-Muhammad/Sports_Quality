...
class Ball_Forming extends CI_Model
{
    ...
    public function getDataTM()
    {
      ...
      $Day = date('d');
        ...
        // ");
				...
				$query = $this->db->query("SELECT    SUM(OutPut) AS OutPut
				FROM            dbo.view_BallForming_machineWiseTM
				WHERE        (EntryDate = CONVERT(DATETIME, '$Year-$Month-$Day 00:00:00', 102))");
    ...
    public function Stationwise()
    {
      ...
      $Day = date('d');
        ...
        $query = $this->db->query("SELECT         MAX(dbo.tbl_BallForming.OutPut) AS OutPut, 
				DATEADD(dd, 0, DATEDIFF(dd, 0, dbo.tbl_BallForming.EntryDate)) AS EntryDate,dbo.tbl_BallForming.Type,
				...
				GROUP BY DATEADD(dd, 0, DATEDIFF(dd, 0, dbo.tbl_BallForming.EntryDate)), dbo.tbl_BallForming.Type,dbo.tbl_BallForming.MID, dbo.tbl_DMMS_Dept_Machine.Name
				HAVING     (dbo.tbl_BallForming.Type = 'LFB') and    (DATEADD(dd, 0, DATEDIFF(dd, 0, dbo.tbl_BallForming.EntryDate)) = CONVERT(DATETIME, '$Year-$Month-$Day 00:00:00', 102))
				ORDER BY dbo.tbl_DMMS_Dept_Machine.Name
        ");
      return  $query->result_array();
    }
    public function StationwiseTM()
    {
      $Month = date('m');
      $Year = date('Y');
      $Day = date('d');
        ...
        $query = $this->db->query("SELECT         MAX(dbo.tbl_BallForming.OutPut) AS OutPut, 
				...
				FROM            dbo.tbl_BallForming INNER JOIN
																 dbo.tbl_DMMS_Dept_Machine ON dbo.tbl_BallForming.MID = dbo.tbl_DMMS_Dept_Machine.DMID
				GROUP BY DATEADD(dd, 0, DATEDIFF(dd, 0, dbo.tbl_BallForming.EntryDate)), dbo.tbl_BallForming.Type,dbo.tbl_BallForming.MID, dbo.tbl_DMMS_Dept_Machine.Name
				HAVING       (dbo.tbl_BallForming.Type = 'TM') and  (DATEADD(dd, 0, DATEDIFF(dd, 0, dbo.tbl_BallForming.EntryDate)) = CONVERT(DATETIME, '$Year-$Month-$Day 00:00:00', 102))
				ORDER BY dbo.tbl_DMMS_Dept_Machine.Name
        ");
      return  $query->result_array();
    }
