...
HAVING        (DateName BETWEEN CONVERT(DATETIME, '$startDate 00:00:00', 102) AND CONVERT(DATETIME, '$endDate 00:00:00', 102))");
    ...
    public function getDataCurrentDate()
    {
        $start_date = date('Y/m/d');
        $end_date = date('Y/m/d');

        $query =  $this->db->query("SELECT        dbo.tbl_PC_Wastages_Info.CarcusType, SUM(dbo.tbl_PC_Wastages_Info.Qty) AS Qty, SUM(dbo.tbl_PC_Wastages_Info.Pass) AS Pass, SUM(dbo.tbl_PC_Wastages_Info.Fail) AS Fail, SUM(dbo.tbl_PC_Wastages_Info.SSQty) 
        AS SSQty, SUM(dbo.tbl_PC_Wastages_Info.PrintQty) AS PrintQty, SUM(dbo.tbl_PC_Wastages_Info.MaterialDefect) AS MaterialDefect, SUM(dbo.tbl_PC_Wastages_Info.PanelForming) AS PanelForming, 
        SUM(dbo.tbl_PC_Wastages_Info.Thermonix) AS Thermonix, SUM(dbo.tbl_PC_Wastages_Info.BallForming) AS BallForming, SUM(dbo.tbl_PC_Wastages_Info.Repair) AS Repair, SUM(dbo.tbl_PC_Wastages_Info.MisPrinting) 
        AS MisPrinting, SUM(dbo.tbl_PC_Wastages_Info.Smearing) AS Smearing, SUM(dbo.tbl_PC_Wastages_Info.Setting) AS Setting, SUM(dbo.tbl_PC_Wastages_Info.Dust) AS Dust, SUM(dbo.tbl_PC_Wastages_Info.BladderPuncture) 
        AS BladderPuncture, SUM(dbo.tbl_PC_Wastages_Info.Fabric) AS Fabric, SUM(dbo.tbl_PC_Wastages_Info.DShape) AS DShape, SUM(dbo.tbl_PC_Wastages_Info.OverSize) AS OverSize, 
        dbo.tbl_PC_Wastages_Info.FactoryCode
FROM            dbo.tbl_PC_Wastages_Info INNER JOIN
        dbo.tbl_Inv_Tran_Date ON dbo.tbl_PC_Wastages_Info.DateId = dbo.tbl_Inv_Tran_Date.DayNo
GROUP BY dbo.tbl_PC_Wastages_Info.CarcusType, dbo.tbl_PC_Wastages_Info.FactoryCode");
        return $query->result_array();
    }
    public function getDateRangeData($start_date, $end_date)
    {
...
FROM            dbo.tbl_PC_Wastages_Info INNER JOIN
        dbo.tbl_Inv_Tran_Date ON dbo.tbl_PC_Wastages_Info.DateId = dbo.tbl_Inv_Tran_Date.DayNo
WHERE        (dbo.tbl_Inv_Tran_Date.DateName BETWEEN CONVERT(DATETIME, '$start_date 00:00:00', 102) AND CONVERT(DATETIME, '$end_date 00:00:00', 102))
GROUP BY dbo.tbl_PC_Wastages_Info.CarcusType, dbo.tbl_PC_Wastages_Info.FactoryCode");
        return $query->result_array();
    }
}
