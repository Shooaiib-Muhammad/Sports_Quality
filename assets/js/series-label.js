...
(function (k) {
	...
	u(
		k,
		"Extensions/SeriesLabel.js",
		[
			k["Core/Animation/AnimationUtilities.js"],
			k["Core/Chart/Chart.js"],
			k["Core/FormatUtilities.js"],
			k["Core/DefaultOptions.js"],
		...
		function (k, u, y, K, E, F, x) {
			...
			function D(b, c, a, f, d, e, r, h) {
				return (
					C(b, c, d, e, r, h) !== C(a, f, d, e, r, h) &&
					C(b, c, a, f, d, e) !== C(b, c, a, f, r, h)
				);
			}
			function A(b, c, a, f, d, e, r, h) {
				return (
					D(b, c, b + a, c, d, e, r, h) ||
					D(b + a, c, b + a, c + f, d, e, r, h) ||
					D(b, c + f, b + a, c + f, d, e, r, h) ||
					D(b, c, b, c + f, d, e, r, h)
				);
			}
			function I(b) {
				if (this.renderer) {
					...
					c.series.forEach(function (f) {
						...
						d.enabled &&
							...
							(f.graph || f.area) &&
							!f.isSeriesBoosting &&
							(c.labelSeries.push(f),
							d.minFontSize &&
								d.maxFontSize &&
								((f.sum = f.yData.reduce(function (a, b) {
									return (a || 0) + (b || 0);
								}, 0)),
								(c.labelSeriesMaxSum = Math.max(c.labelSeriesMaxSum, f.sum))),
							"load" === b.type &&
								(a = Math.max(a, G(f.options.animation).duration)),
							r &&
								("undefined" !== typeof r[0].plotX
									? e.animate({ x: r[0].plotX + r[1], y: r[0].plotY + r[2] })
									: e.attr({ opacity: 0 })));
					});
					c.seriesLabelTimer = L(
						function () {
				...
				if (this.xAxis || this.yAxis) {
					...
					if (
						...
						for (m = c.length, k = 0; k < m; k += 1) {
							...
							if (
								0 < k &&
								((d = Math.abs(l.chartX - f.chartX)),
								(g = Math.abs(l.chartY - f.chartY)),
								(d = Math.max(d, g)),
								16 < d)
							)
								for (d = Math.ceil(d / 16), g = 1; g < d; g += 1)
									b({
										chartX: f.chartX + (g / d) * (l.chartX - f.chartX),
										chartY: f.chartY + (g / d) * (l.chartY - f.chartY),
										chartCenterY:
											f.chartCenterY +
											(g / d) * (l.chartCenterY - f.chartCenterY),
			...
			E.prototype.checkClearPoint = function (b, c, a, f) {
				...
				for (z = 0; z < d.boxesToAvoid.length; z += 1) {
					...
					var l = c;
					var t = c + a.height;
					if (!(b > g.right || m < g.left || l > g.bottom || t < g.top))
						return !1;
				}
				for (z = 0; z < d.series.length; z += 1)
					if (((l = d.series[z]), (g = l.interpolatedPoints), l.visible && g)) {
						for (m = 1; m < g.length; m += 1) {
							if (
								g[m].chartX >= b - 16 &&
								g[m - 1].chartX <= b + a.width + 16
							) {
								if (
									A(
										b,
						...
						if (
							...
							this === l &&
							((f && !p) ||
								h < Math.pow(this.options.label.connectorNeighbourDistance, 2))
						) {
							for (m = 1; m < g.length; m += 1)
								if (
									((p = Math.min(
										Math.pow(b + a.width / 2 - g[m].chartX, 2) +
											Math.pow(c + a.height / 2 - g[m].chartY, 2),
										Math.pow(b - g[m].chartX, 2) + Math.pow(c - g[m].chartY, 2),
										Math.pow(b + a.width - g[m].chartX, 2) +
											Math.pow(c - g[m].chartY, 2),
										Math.pow(b + a.width - g[m].chartX, 2) +
											Math.pow(c + a.height - g[m].chartY, 2),
										Math.pow(b - g[m].chartX, 2) +
											Math.pow(c + a.height - g[m].chartY, 2)
									)),
									p < n)
								) {
				...
				return !f || p
					? { x: b, y: c, weight: h - (w ? n : 0), connectorPoint: w }
					: !1;
			};
			u.prototype.drawSeriesLabels = function () {
				...
				b.series.forEach(function (a) {
					...
					if (d && (a.xAxis || a.yAxis)) {
						...
						if (x && !p) {
							p = [
								a.xAxis.toPixels(a.xData[0]),
								a.xAxis.toPixels(a.xData[a.xData.length - 1]),
							];
						...
						if (a.visible && !a.isSeriesBoosting && t) {
							v ||
								((v = a.name),
								"string" === typeof d.format
									? (v = M(d.format, a, b))
									: d.formatter && (v = d.formatter.call(a)),
								(a.labelBySeries = v =
									b.renderer
										.label(v, 0, -9999, "connector")
										.addClass(
								...
								b.renderer.styledMode ||
									(v.css(
										J(
											{ color: x ? b.renderer.getContrast(a.color) : a.color },
											d.style || {}
										)
									),
									v.attr({
								...
								h && n && v.css({ fontSize: a.labelFontSize(h, n) }),
								v.attr({ padding: 0, zIndex: 3 }).add());
							e = v.getBBox();
							e.width = Math.round(e.width);
							for (p = t.length - 1; 0 < p; --p)
								x
									? ((h = t[p].chartX - e.width / 2),
									  (n = t[p].chartCenterY - e.height / 2),
									  c(h, n, e) && (q = a.checkClearPoint(h, n, e)))
									: ((h = t[p].chartX + 3),
									  (n = t[p].chartY - e.height - 3),
									  c(h, n, e) && (q = a.checkClearPoint(h, n, e, !0)),
									  q && w.push(q),
									  (h = t[p].chartX + 3),
									  (n = t[p].chartY + 3),
									  c(h, n, e) && (q = a.checkClearPoint(h, n, e, !0)),
									  q && w.push(q),
									  (h = t[p].chartX - e.width - 3),
									  (n = t[p].chartY + 3),
									  c(h, n, e) && (q = a.checkClearPoint(h, n, e, !0)),
									  q && w.push(q),
									  (h = t[p].chartX - e.width - 3),
							...
							if (d.connectorAllowed && !w.length && !x)
								for (h = u + m - e.width; h >= u; h -= 16)
									for (n = g; n < g + l - e.height; n += 16)
										(q = a.checkClearPoint(h, n, e, !0)) && w.push(q);
							if (w.length) {
								if (
									(w.sort(function (a, b) {
									...
									(t = Math.sqrt(
										Math.pow(Math.abs(q.x - (v.x || 0)), 2) +
											Math.pow(Math.abs(q.y - (v.y || 0)), 2)
									)) &&
										a.labelBySeries &&
										((w = {
											opacity: b.renderer.forExport ? 1 : 0,
										...
										(d = { opacity: 1 }),
										10 >= t && ((d = { x: w.x, y: w.y }), (w = {})),
										(t = void 0),
										k && ((t = G(a.options.animation)), (t.duration *= 0.2)),
										a.labelBySeries
											.attr(
												J(w, {
													anchorX:
														q.connectorPoint && q.connectorPoint.plotX + u,
													anchorY:
														q.connectorPoint && q.connectorPoint.plotY + g,
												})
											)
											.animate(d, t),
										(a.options.kdNow = !0),
										a.buildKDTree(),
										(a = a.searchPoint({ chartX: q.x, chartY: q.y }, !0))))
								)
									v.closest = [a, q.x - (a.plotX || 0), q.y - (a.plotY || 0)];
